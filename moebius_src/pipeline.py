# Vendored + adapted from hustvl/Moebius removal/v1_2/pipeline.py (see NOTICE).
# The denoising math, preprocessing and post-processing are kept identical to
# upstream RemovalSDXLPipeline_BatchMode. Changes:
#   - dropped: migan hooks, matplotlib latent visualization, tqdm, dead
#     `no_cfg_after_step` parameter
#   - added: `seed` parameter (upstream seeded via `retry`; its CLI --seed was
#     never wired through) and `step_callback(step, total)` called once per
#     denoise step (raising inside the callback aborts cleanly — used for
#     ComfyUI's progress bar + interrupt)
from copy import deepcopy
import random
from collections import UserDict  # noqa: F401  (kept for upstream parity)
...
class MoebiusPipeline:
    ...
    def _denoise_preprocess(self, input_image: I.Image, input_mask: I.Image) -> List[Tensor]:
    ...
    def __call__(
            ...
            noise_offset=0.0357,
            step_callback=None):
        # Upstream parity: global seeding (retry -> seed), torch.randn_like has
        # no generator arg in this code path.
        random.seed(seed)
